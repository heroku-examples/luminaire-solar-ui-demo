...
import {
  ...
  closeEventSource,
} from '../services/eventSourceService.js';
import {
  sendTypingIndicator,
  sendTextMessage,
  getContinuityJwt,
...
import * as ConversationEntryUtil from '../helpers/conversationEntryUtil.js';
import {
...
import { util } from '../helpers/common.js';
import { prechatUtil } from '../helpers/prechatUtil.js';
import Prechat from './prechat.jsx';
...
export default function Conversation(props) {
  ...
  let [conversationStatus, setConversationStatus] = useState(
  ...
  useEffect(() => {
    ...
    props.uiReady(true);
    conversationStatePromise = props.isExistingConversation
      ? handleExistingConversation()
      : handleNewConversation();
    conversationStatePromise.then(() => {
      handleSubscribeToEventSource().catch(() => {
   ...
   *
   * 1. Fetch an Authenticated Access Token i.e. Messaging JWT.
   * 2. Create a new conversation.
   * @returns {Promise}
   */
  function handleNewConversation() {
    return handleGetAuthenticatedJwt()
      .then(() => {
        if (prechatUtil.shouldDisplayPrechatForm()) {
          console.log(
            'Pre-Chat is enabled. Continuing to render a Pre-Chat form.'
          );
          setShowPrechatForm(true);
          return;
        }
        console.log(
          'Pre-Chat is not enabled. Continuing to create a new conversation.'
        );
        return handleCreateNewConversation()
          .then(() => {
            console.log(
              `Completed initializing a new conversation with conversationId: ${getConversationId()}`
            );
          })
          .catch((err) => {
            console.error(`${err}`);
        ...
        return handleListConversations()
          .then(() => {
            ...
            handleListConversationEntries()
              .then(
                console.log(
                  `Successfully retrieved entries for the current conversation: ${getConversationId()}`
                )
              )
              .catch((err) => {
                console.error(`${err}`);
  ...
  /**
   * Handles fetching an Authenticated Access Token i.e. Messaging JWT.
   *
   * 1. If a JWT already exists, simply return.
   * 2. Makes a request to Authenticated Access Token endpoint.
  ...
  function handleGetAuthenticatedJwt() {
    if (getJwt()) {
      console.warn(
        'Messaging access token (JWT) already exists in the web storage. Discontinuing to create a new Authenticated access token.'
      );
        ...
        if (typeof response === 'object') {
          ...
          setDeploymentConfiguration(
            response.context &&
              ...
              response.context.configuration.embeddedServiceConfig
          );
        }
      })
      .catch((err) => {
        console.error(
          `Something went wrong in fetching an Authenticated access token: ${err && err.message ? err.message : err}`
        );
  ...
  function handleCreateNewConversation(routingAttributes) {
    if (
      ...
      console.warn(
        'Cannot create a new conversation while a conversation is currently open.'
      );
    ...
    return createConversation(getConversationId(), routingAttributes)
      .then(() => {
        console.log(
          `Successfully created a new conversation with conversation-id: ${getConversationId()}`
        );
      ...
      .catch((err) => {
        console.error(
          `Something went wrong in creating a new conversation with conversation-id: ${getConversationId()}: ${err && err.message ? err.message : err}`
        );
        handleMessagingErrors(err);
        cleanupMessagingData();
        props.showMessagingWindow(false);
  ...
  function handleGetContinuityJwt() {
    return getContinuityJwt()
      ...
      .catch((err) => {
        console.error(
          `Something went wrong in fetching a Continuation Access Token: ${err && err.message ? err.message : err}`
        );
        handleMessagingErrors(err);
        props.showMessagingWindow(false);
  ...
  function handleListConversations() {
    return listConversations()
      .then((response) => {
        if (
          response &&
          response.openConversationsFound > 0 &&
          response.conversations.length
        ) {
          const openConversations = response.conversations;
          if (openConversations.length > 1) {
            console.warn(
              `Expected the user to be participating in 1 open conversation but instead found ${openConversations.length}. Loading the conversation with latest startTimestamp.`
            );
            openConversations.sort(
              (conversationA, conversationB) =>
                conversationB.startTimestamp - conversationA.startTimestamp
            );
          }
      ...
      .catch((err) => {
        console.error(
          `Something went wrong in fetching a list of conversations: ${err && err.message ? err.message : err}`
        );
        handleMessagingErrors(err);
        ...
        if (Array.isArray(response)) {
          response.reverse().forEach((entry) => {
            const conversationEntry =
              generateConversationEntryForCurrentConversation(entry);
            ...
            switch (conversationEntry.entryType) {
              case CONVERSATION_CONSTANTS.EntryTypes.CONVERSATION_MESSAGE:
                conversationEntry.isEndUserMessage =
                  ConversationEntryUtil.isMessageFromEndUser(conversationEntry);
              ...
              default:
                console.log(
                  `Unrecognized conversation entry type: ${conversationEntry.entryType}.`
                );
            }
          });
        } else {
          console.error(
            `Expecting a response of type Array from listConversationEntries but instead received: ${response}`
          );
        }
      })
      .catch((err) => {
        console.error(
          `Something went wrong while processing entries from listConversationEntries response:  ${err && err.message ? err.message : err}`
        );
        handleMessagingErrors(err);
        throw new Error(
          'Failed to list the conversation entries for the current conversation.'
        );
      });
  }
  ...
  function handleSubscribeToEventSource() {
    return subscribeToEventSource({
      [CONVERSATION_CONSTANTS.EventTypes.CONVERSATION_MESSAGE]:
        handleConversationMessageServerSentEvent,
      [CONVERSATION_CONSTANTS.EventTypes.CONVERSATION_ROUTING_RESULT]:
        handleRoutingResultServerSentEvent,
      [CONVERSATION_CONSTANTS.EventTypes.CONVERSATION_PARTICIPANT_CHANGED]:
        handleParticipantChangedServerSentEvent,
      [CONVERSATION_CONSTANTS.EventTypes.CONVERSATION_TYPING_STARTED_INDICATOR]:
        handleTypingStartedIndicatorServerSentEvent,
      [CONVERSATION_CONSTANTS.EventTypes.CONVERSATION_TYPING_STOPPED_INDICATOR]:
        handleTypingStoppedIndicatorServerSentEvent,
      [CONVERSATION_CONSTANTS.EventTypes.CONVERSATION_DELIVERY_ACKNOWLEDGEMENT]:
        handleConversationDeliveryAcknowledgementServerSentEvent,
      [CONVERSATION_CONSTANTS.EventTypes.CONVERSATION_READ_ACKNOWLEDGEMENT]:
        handleConversationReadAcknowledgementServerSentEvent,
      [CONVERSATION_CONSTANTS.EventTypes.CONVERSATION_CLOSE_CONVERSATION]:
        handleCloseConversationServerSentEvent,
    })
      .then(() => {
  ...
  function generateConversationEntryForCurrentConversation(parsedEventData) {
    const conversationEntry =
      ConversationEntryUtil.createConversationEntry(parsedEventData);
    ...
    console.log(
      `Current conversation-id: ${getConversationId()} does not match the conversation-id in server sent event: ${parsedEventData.conversationId}. Ignoring the event.`
    );
    return undefined;
  }
  ...
  function handleConversationMessageServerSentEvent(event) {
    try {
      console.log(
        `Successfully handling conversation message server sent event.`
      );
      ...
      const parsedEventData =
        ConversationEntryUtil.parseServerSentEventData(event);
      const conversationEntry =
        generateConversationEntryForCurrentConversation(parsedEventData);
      if (!conversationEntry) {
        return;
      }
      ...
      if (ConversationEntryUtil.isMessageFromEndUser(conversationEntry)) {
        ...
        console.log(
          `Successfully received a message from ${conversationEntry.actorType}`
        );
      }
      ...
      console.error(
        `Something went wrong in handling conversation message server sent event: ${err}`
      );
    }
  }
  ...
  function handleRoutingResultServerSentEvent(event) {
    try {
      ...
      if (
        conversationEntry.messageType ===
        CONVERSATION_CONSTANTS.RoutingTypes.INITIAL
      ) {
        ...
        switch (conversationEntry.content.failureType) {
          ...
          default:
            console.error(
              `Unrecognized initial routing failure type: ${conversationEntry.content.failureType}`
            );
        }
        // Handle when a conversation is being transferred.
      } else if (
        conversationEntry.messageType ===
        CONVERSATION_CONSTANTS.RoutingTypes.TRANSFER
      ) {
        switch (conversationEntry.content.failureType) {
          case CONVERSATION_CONSTANTS.RoutingFailureTypes.NO_ERROR:
          ...
          default:
            console.error(
              `Unrecognized transfer routing failure type: ${conversationEntry.content.failureType}`
            );
        }
      } else {
        console.error(
          `Unrecognized routing type: ${conversationEntry.messageType}`
        );
      }
    } catch (err) {
      console.error(
        `Something went wrong in handling routing result server sent event: ${err}`
      );
    }
  }
  ...
  function handleParticipantChangedServerSentEvent(event) {
    try {
      console.log(
        `Successfully handling participant changed server sent event.`
      );
      // Update in-memory to the latest lastEventId
      if (event && event.lastEventId) {
        setLastEventId(event.lastEventId);
      ...
      console.error(
        `Something went wrong in handling participant changed server sent event: ${err}`
      );
    }
  }
  ...
  function handleTypingStartedIndicatorServerSentEvent(event) {
    try {
      ...
      if (getConversationId() === parsedEventData.conversationId) {
        const senderName =
          ConversationEntryUtil.getSenderDisplayName(parsedEventData) ||
          ConversationEntryUtil.getSenderRole(parsedEventData);
        const typingParticipantTimer =
          currentTypingParticipants[senderName] &&
          currentTypingParticipants[senderName].countdownTimer;
        ...
        if (
          ...
          console.log(
            `Successfully handling typing started indicator server sent event.`
          );
          ...
          if (typingParticipantTimer) {
            ...
            currentTypingParticipants[senderName] = {
              countdownTimer: new CountdownTimer(
                ...
                CLIENT_CONSTANTS.TYPING_INDICATOR_DISPLAY_TIMEOUT,
                Date.now()
              ),
      ...
      console.error(
        `Something went wrong in handling typing started indicator server sent event: ${err}`
      );
    }
  }
  ...
  function handleTypingStoppedIndicatorServerSentEvent(event) {
    try {
      ...
      console.error(
        `Something went wrong in handling typing stopped indicator server sent event: ${err}`
      );
    }
  }
  ...
  function handleConversationDeliveryAcknowledgementServerSentEvent(event) {
    try {
      console.log(
        `Successfully handling conversation delivery acknowledgement server sent event.`
      );
      // Update in-memory to the latest lastEventId
      if (event && event.lastEventId) {
        setLastEventId(event.lastEventId);
      ...
      if (getConversationId() === parsedEventData.conversationId) {
        if (
          parsedEventData.conversationEntry &&
          parsedEventData.conversationEntry.relatedRecords.length &&
          parsedEventData.conversationEntry.entryPayload.length
        ) {
          // Store acknowledgement timestamp and identifier of message that was delivered.
          const deliveredMessageId =
            parsedEventData.conversationEntry.relatedRecords[0];
          const deliveryAcknowledgementTimestamp = JSON.parse(
            parsedEventData.conversationEntry.entryPayload
            ...
            .filter((conversationEntry) => {
              if (conversationEntry.messageId === deliveredMessageId) {
                ...
                conversationEntry.deliveryAcknowledgementTimestamp =
                  deliveryAcknowledgementTimestamp;
              }
            });
        }
      }
      ...
      console.error(
        `Something went wrong in handling conversation delivery acknowledgement server sent event: ${err}`
      );
    }
  }
  ...
  function handleConversationReadAcknowledgementServerSentEvent(event) {
    try {
      console.log(
        `Successfully handling conversation read acknowledgement server sent event.`
      );
      // Update in-memory to the latest lastEventId
      if (event && event.lastEventId) {
        setLastEventId(event.lastEventId);
      ...
      if (getConversationId() === parsedEventData.conversationId) {
        if (
          ...
          const readMessageId =
            parsedEventData.conversationEntry.relatedRecords[0];
          const readAcknowledgementTimestamp = JSON.parse(
            parsedEventData.conversationEntry.entryPayload
          ).acknowledgementTimestamp;
      ...
      console.error(
        `Something went wrong in handling conversation read acknowledgement server sent event: ${err}`
      );
    }
  }
  ...
  function handleCloseConversationServerSentEvent(event) {
    try {
      console.log(
        `Successfully handling close conversation server sent event.`
      );
      // Update in-memory to the latest lastEventId
      if (event && event.lastEventId) {
        setLastEventId(event.lastEventId);
      ...
      console.error(
        `Something went wrong while handling conversation closed server sent event in conversation ${getConversationId()}: ${err}`
      );
    }
  }
  ...
  function handleSendTextMessage(
    ...
    return sendTextMessage(
      ...
      console.error(
        `Something went wrong while sending a message to conversation ${conversationId}: ${err}`
      );
      setFailedMessage(
        Object.assign(
          {},
  ...
  function endConversation() {
    if (
      ...
      return closeConversation(getConversationId())
        .then(() => {
          console.log(
            `Successfully closed the conversation with conversation-id: ${getConversationId()}`
          );
        })
        .catch((err) => {
          console.error(
            `Something went wrong in closing the conversation with conversation-id ${getConversationId()}: ${err}`
          );
        })
        .finally(() => {
          cleanupMessagingData();
  ...
  function cleanupMessagingData() {
    closeEventSource()
      ...
      .catch((err) => {
        console.error(
          `Something went wrong in closing the Event Source (SSE): ${err}`
        );
      });
    ...
    if (typeof err === 'object') {
      if (err.status) {
        switch (err.status) {
          ...
          case 400:
            console.error(
              `Invalid request parameters. Please check your data before retrying: ${err.message}`
            );
            break;
          case 404:
            console.error(
              `Resource not found. Please check your data before retrying: ${err.message}`
            );
            break;
          case 429:
            console.warn(
              `Too many requests issued from the app. Try again in sometime: ${err.message}`
            );
            break;
          /**
          ...
          case 417:
            if (
              prechatUtil.shouldDisplayPrechatForm() &&
              prechatUtil.shouldDisplayPrechatEveryMessagingSession()
            ) {
              console.log(
                'Pre-Chat configured to show for every new messaging session. Continuing to display the Pre-Chat form.'
              );
              setShowPrechatForm(true);
            }
          case 500:
            console.error(
              `Something went wrong in the request, please try again: ${err.message}`
            );
            break;
          default:
  ...
  function handlePrechatSubmit(prechatData) {
    ...
    if (failedMessage) {
      prechatSubmitPromise = handleSendTextMessage(
        getConversationId(),
        failedMessage.value,
  ...
  return (
    <>
      ...
      {showPrechatForm && <Prechat prechatSubmit={handlePrechatSubmit} />}
    </>
  );
}
