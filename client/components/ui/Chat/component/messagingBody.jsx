...
export default function MessagingBody({
   ...
   * @returns {string}
   */
  const conversationEntriesListView = conversationEntries.map(
    (conversationEntry) => (
      <ConversationEntry
  ...
  function generateConversationStartTimeText() {
    if (conversationEntries.length) {
      const conversationStartTimestamp =
        conversationEntries[0].transcriptedTimestamp;
  ...
  return (
    <div className="messagingBody">
      {conversationEntries.length > 0 && (
        <p className="conversationStartTimeText">
      ...
      {conversationStatus ===
        CONVERSATION_CONSTANTS.ConversationStatus.CLOSED_CONVERSATION && (
        <p className="conversationEndTimeText">
