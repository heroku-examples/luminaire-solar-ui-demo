...
export default function MessagingInputFooter(props) {
  ...
  function handleSendTypingIndicator(typingIndicator) {
    ...
    props
      .sendTypingIndicator(conversationId, typingIndicator)
      .then(() => {
        console.log(
          `Successfully sent ${typingIndicator} to conversation: ${conversationId}`
        );
      })
      .catch((error) => {
        console.error(
          `Something went wrong while sending a typing indicator to conversation ${conversationId}: ${error}`
        );
      });
  }
