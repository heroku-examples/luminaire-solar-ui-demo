...
export const prechatUtil = {
  ...
  getPrechatFormChoiceList(choiceListId) {
    ...
    const choiceList =
      choiceListConfig &&
      choiceListConfig.choiceList &&
      choiceListConfig.choiceList.length &&
      choiceListConfig.choiceList.filter((choiceList) => {
  ...
  getPrechatDisplayFrequency() {
    ...
    return (
      prechatFormData &&
      prechatFormData.length &&
      prechatFormData[0] &&
      prechatFormData[0].displayContext
    );
  },
  ...
  shouldDisplayPrechatForm() {
    ...
    const visiblePrechatFormFields =
      prechatFormFields &&
      prechatFormFields.filter((field) => {
  ...
  shouldDisplayPrechatEveryMessagingSession() {
    ...
    return (
      this.getPrechatDisplayFrequency() ===
      DEPLOYMENT_CONFIGURATION_CONSTANTS.PRECHAT_DISPLAY_EVERY_SESSION
    );
  },
};
