 ...
 */
const handleEventSourceListeners = (
  listenerOperationName,
  eventListenerMap
) => {
  ...
  if (
    typeof listenerOperationName !== 'string' ||
    !listenerOperationName.length
  ) {
    throw new Error(
...
export const subscribeToEventSource = (eventListenerMap) => {
  ...
  return new Promise((resolve, reject) => {
    try {
       ...
       */
      createEventSource(
        getSalesforceMessagingUrl().concat(
          `/eventrouter/v1/sse?_ts=${Date.now()}`
        ),
        eventListenerMap
      )
        .then(resolve, (error) => {
