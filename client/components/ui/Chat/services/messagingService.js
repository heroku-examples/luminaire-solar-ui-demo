 ...
 * @returns {Promise}
 */
function getAuthenticatedAccessToken(customerIdentityToken) {
  const orgId = getOrganizationId();
  const esDeveloperName = getDeploymentDeveloperName();
  const messagingUrl = getSalesforceMessagingUrl();
  const capabilitiesVersion = APP_CONSTANTS.APP_CAPABILITIES_VERSION;
  const platform = APP_CONSTANTS.APP_PLATFORM;
  const apiPath = `${messagingUrl}/iamessage/api/v2/authorization/authenticated/access-token`;
  const authorizationType = 'JWT';
  return sendFetchRequest(
    apiPath,
    'POST',
    'cors',
...
function getContinuityJwt() {
  ...
  return sendFetchRequest(apiPath, 'GET', 'cors', null, null).then(
    (response) => {
      if (!response.ok) {
        throw response;
      }
 ...
 * @returns {Promise}
 */
function listConversationEntries(
  conversationId,
  startTimestamp,
  endTimestamp,
  direction
  ...
  return sendFetchRequest(apiPath, 'GET', 'cors', null, null)
    .then((response) => {
      if (!response.ok) {
        throw response;
      }
    ...
    .then((responseJson) => {
      ...
      responseJson.conversationEntries.forEach((conversationEntry) => {
        conversationEntry.entryPayload = JSON.stringify(
...
function sendTextMessage(
  ...
  return sendFetchRequest(apiPath, 'POST', 'cors', null, {
    message: {
...
function closeConversation(conversationId) {
  ...
  return sendFetchRequest(apiPath, 'DELETE', 'cors', null, null);
}
